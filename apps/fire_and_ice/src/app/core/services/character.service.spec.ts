...
describe('CharacterService', () => {
  ...
  it('should get a character by character url', () => {
    characterService.findOne(perCharacterUrl).subscribe((character) => {
      ...
      expect(character?.name)
        .withContext('name of character must be equal to ')
        .toEqual('Braavosi');
    });
    const request = httpClient.expectOne(perCharacterUrl);
    const httpRequest = request.request;
    expect(httpRequest.url).toEqual(perCharacterUrl);
    expect(httpRequest.method)
      .withContext('Request must be a GET request')
  ...
  it('should get all characters', () => {
    characterService.findAll().subscribe((characters) => {
      ...
      expect(characters.length)
        .withContext('Incorrect number of characters')
        .toEqual(1);
      expect(
        characters.find((character) => character.name === 'Braavosi')?.name
      )
        .withContext('Name of character must be Braavosi')
        .toBe('Braavosi');
    });
  ...
  it('should confirm if pagination is correct', () => {
    ...
    const request = httpClient.expectOne(url);
    const httpRequest = request.request;
    expect(httpRequest.params.get('page'))
      .withContext('the page  must be 1')
      .toEqual('1');
    expect(httpRequest.params.get('pageSize'))
      .withContext('the page size must be ')
      .toEqual('10');
  ...
  it('should confirm if filter is correct', () => {
    const url = '/characters?name=Braavosi';
    characterService.findAll(undefined, { name: 'Braavosi' }).subscribe();
    const request = httpClient.expectOne(url);
    const httpRequest = request.request;
    expect(httpRequest.params.get('name'))
      .withContext('filter param must be name')
      .toEqual('Braavosi');
    request.flush({ success: true });
  });
