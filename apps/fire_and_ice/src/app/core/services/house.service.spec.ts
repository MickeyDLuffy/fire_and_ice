...
describe('HouseService', () => {
  ...
  it('should get a house by house url', () => {
    service.findOne(byIdUrl).subscribe((house) => {
      ...
      expect(house?.region)
        .withContext('region of house must be equal to ')
        .toEqual('The Westerlands');
    });
    const request = httpController.expectOne(byIdUrl);
  ...
  it('should get all houses', () => {
    service.findAll().subscribe((houses) => {
      ...
      expect(houses.length)
        .withContext('Incorrect number of houses')
        .toEqual(1);
      expect(houses.find((house) => house.name === 'House Algood')?.name)
        .withContext('Name of house must be House Algood')
        .toBe('House Algood');
    });
  ...
  it('should confirm if pagination is correct', () => {
    const url = '/houses?page=1&pageSize=10';
  ...
  it('should confirm if filter is correct', () => {
    const url = `/houses?region=Westerlands`;
    service.findAll(undefined, { region: 'Westerlands' }).subscribe();
    const request = httpController.expectOne(url);
    const httpRequest = request.request;
    expect(httpRequest.params.get('region'))
      .withContext('filter param must be region')
      .toEqual('Westerlands');
    request.flush({ success: true });
  });
