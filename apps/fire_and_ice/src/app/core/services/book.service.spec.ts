...
describe('BookService', () => {
  ...
  it('should find one book by url', function () {
    bookService.findOne(url).subscribe((actualBook) => {
      expect(actualBook).withContext('No book item returned').toBeTruthy();
      expect(actualBook?.name)
        .withContext('Incorrect name for book')
        .toEqual('A Game of Thrones');
    });
    const request = httpController.expectOne(url);
    expect(request.request.method)
      .withContext('This must be a GET request')
      .toEqual('GET');
    request.flush(book);
  });
  it('should get all books', () => {
    ...
    bookService.findAll().subscribe((books) => {
      ...
      expect(
        books.find((book) => book.name === 'A Game of Thrones')?.name
  ...
  it('should check errors for getting a book by url', function () {
    ...
    httpController.expectOne(url).error(errorEvent, { status, statusText });
    if (!actualError) {
  ...
  it('should confirm if pagination is correct', () => {
    ...
    expect(httpRequest.params.get('pageSize'))
      .withContext('the page sixe must be ')
      .toEqual('10');
